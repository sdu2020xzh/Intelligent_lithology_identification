...
class mAP_FRCNN(FRCNN):
    _defaults = {
        "model_path": 'logs/Epoch100-Total_Loss0.0428-Val_Loss0.0489.pth',
...
def get_gt_txt(model_path):
    frcnn = mAP_FRCNN(model_path)
...
def get_map(path):
    ...
    with open(results_files_path + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            if len(prec) > 0:
                ...
                map_arr.append(ap)
                if np.isnan(F1[score05_idx]):
                    f1_arr.append(0)
                else:
                    f1_arr.append(F1[score05_idx])
                if np.isnan(rec[score05_idx]):
                    r_arr.append(0)
                else:
                    r_arr.append(rec[score05_idx] * 100)
                if np.isnan(prec[score05_idx]):
                    p_arr.append(0)
                else:
                    p_arr.append(prec[score05_idx] * 100)
            else:
    ...
    with open('result.txt', 'a+') as f:
        f.write('\n{},{:.3f},{:.3f},{:.3f},{:.3f}'.format(path.split('\\')[1][5:], sum(map_arr) / len(map_arr),
                                                          sum(f1_arr) / len(f1_arr), sum(r_arr) / len(r_arr),
                                                          sum(p_arr) / len(p_arr)))
...
#     f.write('epoch,map,f1,recall,precision')

for i in tqdm(sorted(os.listdir('logs'), key=lambda x:int(x.split('-')[0][5:]))[70:]):
    since = time.time()
    get_gt_txt('logs\{}'.format(i))
    get_map('map_result\{}'.format(i.split('-')[0]))
    print('use time:{:.5f}'.format(time.time() - since))